...
class FidoCeremonyService {
    ...
    UUID create(FidoCeremonyEntity.Type type, String username, String requestJson) {
        ...
        ceremonies.save(
                new FidoCeremonyEntity(
                        ...
                        username,
                        requestJson,
                        Instant.now(clock).plus(properties.challengeTtl())));
        log.info(
                "event=fido_ceremony_started ceremonyId={} type={} username={}",
                id,
                type,
                username);
        return id;
    ...
    FidoCeremonyEntity consume(UUID id, FidoCeremonyEntity.Type expectedType) {
        ...
        if (ceremony.getType() != expectedType
                || ceremony.isExpired(now)
                || !ceremony.isPending()) {
            log.info(
                    "event=fido_ceremony_rejected ceremonyId={} expectedType={}", id, expectedType);
